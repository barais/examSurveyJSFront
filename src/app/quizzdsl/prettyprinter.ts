...
import { matcher, Sentence, Text, Radio, Dropdown, Checkboxes, File, ImagePicker, BooleanEntry, SignaturePad, Multipletext, Rating, StaticImage, Html, Datepicker, Barrating, SortableJS, Nouislider, IPaddress, Email, CKEditor, EmotionsRatings, UML, Question, QuestionSeparator, PageSeparator } from "./model";
...
export class SuerveyJSPrinter {
  ...
  isRating = false;
  isText = false;
  constructor(private titre: string, private consigne: string, private tmpsenseconde: number) {
  ...
  print(res: string): string {
    return this.printParse(quizzdsl({
      text: res
      , index: 0
    }));
  }
    ...
    "questions": [`;
      t.value.forEach((t1, index1) => {
        t1.forEach((t2, index2) => {
          if (!(index1 === t.value.length - 1 && index2 === t1.length - 1)) {
